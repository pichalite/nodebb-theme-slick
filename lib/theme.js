	...
	SocketPlugins.slick = {
 	...
 	function sendEvent(data, eventName, callback) {
 		async.series({
			...
			websockets.in('topic_' + data.tid).emit(eventName, {pid: data.pid, uid: data.uid, reaction: data.reaction, reactionCount: results.reactionCount, usernames: results.usernames, reactionImage: emojiParser.parse(':' + data.reaction + ':').replace('title="' + data.reaction + '"', '')});
 			callback();
		});
 	}
