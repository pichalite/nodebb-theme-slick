	...
	SocketPlugins.slick = {
 		addReaction: function(socket, data, callback) {
 			...
 			meta.settings.get('slick', function(err, settings) {
 				...
 				db.setCount('pid:' + data.pid + ':reactions', function (err, totalReactions) {
 	...
 	function sendEvent(data, eventName, callback) {
 		async.series({
			...
			websockets.in('topic_' + data.tid).emit(eventName, {pid: data.pid, uid: data.uid, reaction: data.reaction, reactionCount: results.reactionCount, totalReactions: results.totalReactions, usernames: results.usernames, reactionImage: emojiParser.parse(':' + data.reaction + ':').replace('title="' + data.reaction + '"', '')});
 			callback();
		});
 	}
